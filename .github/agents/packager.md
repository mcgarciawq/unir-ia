...
## 🎯 Primary Goal
Generate the final `m4_proyecto_carmen_garcia-peral.zip` in the **project root** while strictly adhering to the exclusion rules defined in `.gitignore`.
...
find . -type f \
  ...
  -not -name "*.pyd" \
  -not -name ".DS_Store" \
  | zip -@ m4_proyecto_carmen_garcia-peral.zip
```

#### PowerShell
```powershell
$exclude = Get-Content .gitignore | Where-Object { $_ -and -not $_.StartsWith('#') }
Get-ChildItem -Path . -Recurse -Exclude $exclude | Compress-Archive -DestinationPath ./m4_proyecto_carmen_garcia-peral.zip -Force
```
